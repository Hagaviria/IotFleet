...
export class WebSocketService {
  ...
  private readonly WS_URL = 'wss://localhost:7162/RealTime';
  ...
  getLocationUpdates(): Observable<RealTimeUpdate> {
    return this.realTimeUpdates$.pipe(filter((update) => !!update.location));
  }

  getSensorDataUpdates(): Observable<RealTimeUpdate> {
    return this.realTimeUpdates$.pipe(filter((update) => !!update.sensorData));
  }

  getAlertUpdates(): Observable<RealTimeUpdate> {
    return this.realTimeUpdates$.pipe(filter((update) => !!update.alert));
  }
